...
import math  # For sqrt in Euclidean distance calculations and exp in SA acceptance probability
...
import os  # For file path handling (e.g., reading waypoints.txt, writing path_sa.txt)
import random  # For generating random tours and 2-opt swaps in Simulated Annealing
...
def solve_tsp(distance_matrix: np.ndarray, N: int) -> tuple[list[int], float]:
    """Solve TSP using Simulated Annealing for near-optimal path.

    Starts with a random tour, applies 2-opt swaps, and accepts changes based on temperature.
    Acceptance probability: exp(-ΔE/T), with geometric cooling (T *= cooling_rate).
    Tuned for fast convergence on N <= 15.
    ...
    # Initialize random tour starting and ending at node 0
    path = list(range(N))
    random.shuffle(path)
    path = [0] + [x for x in path if x != 0] + [0]  # Ensure starts and ends at 0
    current_cost = compute_path_cost(distance_matrix, path)
    best_path = path[:]
    best_cost = current_cost
    
    # Simulated Annealing parameters
    initial_temp = 1000.0  # Starting temperature for accepting worse solutions
    cooling_rate = 0.995   # Geometric cooling factor per iteration
    ...
    for _ in range(max_iterations):
        ...
        if temp < 0.01:
            break
        # Generate 2-opt swap (reverse segment between indices i and j)
        i, j = sorted(random.sample(range(1, N), 2))
        new_path = path[:i] + path[i:j+1][::-1] + path[j+1:]
        new_cost = compute_path_cost(distance_matrix, new_path)
        
        # Calculate cost difference (ΔE)
        delta = new_cost - current_cost
        # Accept new path if better or with probability exp(-ΔE/T)
    ...
    try:
        ...
        raise ValueError(f"Simulated Annealing path invalid: {str(e)}")
...
def explain_path(waypoints: list[tuple[int, float, float, float]], path: list[int], distance_matrix: np.ndarray) -> float:
    ...
    print("The path minimizes the total Euclidean distance (fuel cost) using Simulated Annealing, "
          "a heuristic that applies 2-opt swaps and accepts worse solutions probabilistically to "
          "escape local minima, tuned for N <= 15.")
...
def visualize_path(waypoints: list[tuple[int, float, float, float]], path: list[int], fuel_cost: float) -> tuple[float, float]:
    ...
    try:
        ...
        ax.set_ylabel('Y')
        ax.set_zlabel('Z')
        ax.set_title(f'Near-Optimal Path (Simulated Annealing, Fuel Cost: {fuel_cost:.2f})')
        ax.legend()
        ...
        # Save plot to file
        output_path = 'traditional approaches/Simulated_Annealing_Implementation/path_visualization_sa.png'
...
def write_output(file_path: str, waypoints: list[tuple[int, float, float, float]], path: list[int], fuel_cost: float) -> None:
    ...
    Args:
        file_path (str): Path to output file (e.g., path_sa.txt).
        waypoints (list[tuple[int, float, float, float]]): List of (id, x, y, z) tuples.
        path (list[int]): List of waypoint indices.
        fuel_cost (float): Total path cost.
...
def main() -> None:
    ...
    4. Solve TSP using Simulated Annealing.
    ...
    try:
        ...
        # Step 7: Write output
        # ------------------------------
        write_output("traditional approaches/Simulated_Annealing_Implementation/path_sa.txt", waypoints, path, fuel_cost)
        print("Output written to path_sa.txt")
