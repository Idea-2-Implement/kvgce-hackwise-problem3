...
import os  # For file path handling (e.g., reading waypoints1.txt, writing path_minkowski_p{p}.txt)
...
def minkowski_3d(p1: Tuple[float, float, float], p2: Tuple[float, float, float], p: float = 3.0) -> float:
    ...
    Args:
        ...
        p (float): Minkowski parameter (p=1: Manhattan, p=2: Euclidean, p=3: default for innovation).

    Returns:
        float: Minkowski distance, computed as (|x2-x1|^p + |y2-y1|^p + |z2-z1|^p)^(1/p).
    """
    # Compute Minkowski distance using the formula: (|x2-x1|^p + |y2-y1|^p + |z2-z1|^p)^(1/p)
    return (abs(p1[0] - p2[0])**p + abs(p1[1] - p2[1])**p + abs(p1[2] - p2[2])**p)**(1/p)

def compute_distance_matrix(waypoints: List[Tuple[int, float, float, float]], p: float = 3.0) -> np.ndarray:
    """Calculate Minkowski distance matrix for waypoints.

    Args:
        waypoints (List[Tuple[int, float, float, float]]): List of (id, x, y, z) tuples.
        p (float): Minkowski parameter for distance calculations.
    ...
    # Compute Minkowski distances for non-diagonal elements
    for i in range(N):
        for j in range(N):
            if i != j:
                ...
                # Calculate Minkowski distance using p-norm
...
def solve_tsp(distance_matrix: np.ndarray, N: int, waypoints: List[Tuple[int, float, float, float]], p: float = 3.0) -> Tuple[List[int], float]:
    """Solve TSP using Held-Karp Dynamic Programming with Minkowski distance.

    Uses bitmask-based dynamic programming to find the optimal tour, minimizing the total Minkowski distance.
    Returns the path and its cost.

    Args:
        distance_matrix (np.ndarray): N x N distance matrix.
        N (int): Number of waypoints.
        waypoints (List[Tuple[int, float, float, float]]): List of (id, x, y, z) tuples for path reconstruction.
        p (float): Minkowski parameter for distance calculations.
    ...
    print(f"Debug: Initializing DP table for p={p}")
    ...
    for mask in range(1, 1 << N):
        if mask & 1 or mask == 0:
            continue  # Skip masks including city 0 or empty mask
    ...
    try:
        ...
        raise ValueError(f"Held-Karp path invalid for p={p}: {str(e)}")
    
    # Log performance for debugging
    print(f"Debug: TSP solved for p={p}: path={path}, cost={cost:.2f}, time={time.time() - start_time:.4f}s, "
...
def explain_path(waypoints: List[Tuple[int, float, float, float]], path: List[int], distance_matrix: np.ndarray, p: float) -> float:
    ...
    Args:
        ...
        path (List[int]): List of waypoint indices (0-indexed).
        distance_matrix (np.ndarray): N x N distance matrix.
        p (float): Minkowski parameter used for distance calculations.

    Returns:
        float: Total path cost, rounded to 2 decimal places.
    """
    # Explain path selection logic
    print(f"\nLogic Behind Chosen Path (p={p}):")
    print(f"The path is computed using Held-Karp Dynamic Programming with Minkowski distance (p={p}), "
          "using bitmasks to track visited waypoints and computing the optimal tour starting from waypoint 1. "
          "p=2 (Euclidean) is used for hackathon scoring, while other p-values (e.g., p=3) are tested for innovation.")
    ...
    for i in range(len(path) - 1):
        ...
        distance = distance_matrix[start_idx, end_idx]
        # Print segment details with Minkowski formula
...
def visualize_path(waypoints: List[Tuple[int, float, float, float]], path: List[int], fuel_cost: float, p: float) -> Tuple[float, float]:
    ...
    try:
        # Initialize 3D plot
        print(f"Debug: Creating 3D plot for p={p}")
        ...
        ax.set_ylabel('Y')
        ax.set_zlabel('Z')
        ax.set_title(f'Minkowski DP Path (p={p}, Fuel Cost: {fuel_cost:.2f})')
        ax.legend()
        
        # Display plot in Jupyter/Kaggle
        print(f"Debug: Displaying plot for p={p}")
        plt.show()
        
        # Save plot to file with p-specific name
        output_path = f'traditional approaches/Minkowski-based_DP_Approach/path_visualization_minkowski_p{p}.png'
        print(f"Debug: Saving plot to {output_path}")
        plt.savefig(output_path, bbox_inches='tight')
        print(f"Debug: Plot saved successfully for p={p}")
        
        # Close figure to free memory
        plt.close(fig)
        print(f"Debug: Figure closed for p={p}")
        ...
        print(f"Error in visualization for p={p}: {str(e)}")
        raise ValueError(f"Error in visualization for p={p}: {str(e)}")
...
def write_output(file_path: str, waypoints: List[Tuple[int, float, float, float]], path: List[int], fuel_cost: float) -> None:
    ...
    Args:
        file_path (str): Path to output file (e.g., path_minkowski_p{p}.txt).
        waypoints (List[Tuple[int, float, float, float]]): List of (id, x, y, z) tuples.
        path (List[int]): List of waypoint indices (0-indexed).
        fuel_cost (float): Total path cost.
...
def main() -> None:
    """Main function to orchestrate TSP solving, testing multiple Minkowski p-values, and reporting optimal results.

    Steps:
    1. Initialize timing and logging.
    2. Read and validate waypoints.
    3. Test multiple Minkowski p-values (1.0 to 100.0).
    4. Solve TSP for each p-value using Held-Karp Dynamic Programming.
    5. Explain and validate paths for each p-value.
    6. Visualize and save plots for each p-value.
    7. Write output files for each p-value.
    8. Identify and report the optimal path (lowest cost).
    9. Save final output and visualization for the optimal path.
    10. Log overall performance metrics.
    """
    try:
        # ------------------------------
        ...
        # ------------------------------
        # Step 3: Test multiple Minkowski p-values
        # ------------------------------
        p_values = [1.0, 2.0, 3.0, 4.0, 6.0, 8.0, 10.0, 20.0, 50.0, 100.0]
        results = []
        print(f"Testing {len(p_values)} Minkowski p-values: {p_values}")
        
        # Iterate over each p-value
        for p in p_values:
            # ------------------------------
            # Step 4: Solve TSP for current p-value
            # ------------------------------
            print(f"\n=== Testing Minkowski Distance with p={p} ===")
            # Compute distance matrix for current p
            distance_matrix = compute_distance_matrix(waypoints, p)
            print(f"Minkowski distance matrix (p={p}) computed")
            
            # Solve TSP using Held-Karp DP
            tsp_start = time.time()
            path, fuel_cost = solve_tsp(distance_matrix, N, waypoints, p)
            tsp_time = time.time() - tsp_start
            print(f"Optimal path computed for p={p}: {path}, cost={fuel_cost:.2f}")
            
            # ------------------------------
            # Step 5: Explain and validate path
            # ------------------------------
            total_cost = explain_path(waypoints, path, distance_matrix, p)
            validate_path(path, N, distance_matrix, total_cost)
            print(f"Path explanation and validation completed for p={p}")
            
            # ------------------------------
            # Step 6: Visualize path
            # ------------------------------
            vis_time, vis_mem_increase = visualize_path(waypoints, path, fuel_cost, p)
            print(f"Visualization generated for p={p}")
            
            # ------------------------------
            # Step 7: Write output
            # ------------------------------
            output_file = f"traditional approaches/Minkowski-based_DP_Approach/path_minkowski_p{p}.txt"
            write_output(output_file, waypoints, path, fuel_cost)
            ...
            results.append({
                ...
                'tsp_time': tsp_time,
                'vis_time': vis_time,
                'vis_mem_increase': vis_mem_increase
            })
            
            # Log performance for current p-value
            print(f"\nPerformance for p={p}:")
        ...
        # ------------------------------
        # Step 8: Identify and report optimal path
        # ------------------------------
        optimal_result = min(results, key=lambda x: x['fuel_cost'])
        optimal_p = optimal_result['p']
        optimal_path = optimal_result['path']
        optimal_cost = optimal_result['fuel_cost']
        print(f"\n=== Optimal Path and Fuel Cost ===")
        print(f"Optimal Minkowski parameter: p={optimal_p}")
        print(f"Optimal Fuel Cost: {optimal_cost:.2f}")
        one_indexed_path = [waypoints[i][0] for i in optimal_path]
        print(f"Optimal Path: {' '.join(map(str, one_indexed_path))}")
        
        # ------------------------------
        # Step 9: Save final output and visualization for optimal path
        # ------------------------------
        write_output("traditional approaches/Minkowski-based_DP_Approach/path_minkowski.txt", waypoints, optimal_path, optimal_cost)
        print("Final output written to path_minkowski.txt")
        
        # Recompute distance matrix for optimal p and re-visualize
        distance_matrix = compute_distance_matrix(waypoints, optimal_p)
        vis_time, vis_mem_increase = visualize_path(waypoints, optimal_path, optimal_cost, optimal_p)
        print("Final visualization generated for optimal path")
        
        # ------------------------------
        # Step 10: Log overall performance
        # ------------------------------
        end_time = time.time()
